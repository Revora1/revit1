...
const searchUsersReplacement = `
    const searchUsers = async () => {
      ...
      try {
        ...
        if (!searchTerm.trim()) {
          // If empty, fetch 10 users to verify database connection
          const qAll = query(usersRef, limit(10));
          const snapAll = await getDocs(qAll);
          const users = snapAll.docs.map(doc => doc.data() as UserProfile);
        ...
        // Show the error on screen by spoofing a user result temporarily for debugging!
