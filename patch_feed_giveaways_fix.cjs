...
const regex = /<button[\s\S]*?onClick=\{\(\) => window\.dispatchEvent\(new CustomEvent\('navigate-inbox'\)\)\}[\s\S]*?className="p-2 bg-zinc-900\/50 backdrop-blur-md rounded-full text-white border border-white\/10 active:scale-95 transition-transform"[\s\S]*?>[\s\S]*?<MessageSquare size=\{20\} \/>[\s\S]*?<\/button>/;
...
code = code.replace(regex, updatedHeader);
