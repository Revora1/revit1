  ...
  const renderPost = ({ item, index }: { item: any, index: number }) => {
    return (
      <View style={[styles.postContainer, { height: containerHeight }]}>
        ...
        {/* Ad Overlay for specific indices */}
        {index > 0 && (index + 1) % 3 === 0 && nativeAd && (
          <View style={styles.adOverlay}>
             <NativeAdView style={styles.adContainerWrapper} nativeAd={nativeAd}>
                  ...
                  <View style={styles.advertiserTextCol}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                      <NativeAsset assetType={NativeAssetType.HEADLINE}>
                        <Text style={styles.advertiserName} numberOfLines={1}>{nativeAd.headline}</Text>
                      </NativeAsset>
                      <View style={styles.sponsoredTagSmall}>
                        <Text style={styles.sponsoredTagSmallText}>✨ SPONSORED</Text>
                      </View>
                    </View>
                  </View>
                </View>
                <NativeAsset assetType={NativeAssetType.CALL_TO_ACTION}>
                  <TouchableOpacity style={styles.adCtaWhite} activeOpacity={0.8}>
                    <Text style={styles.adCtaWhiteText}>✓ {nativeAd.callToAction || 'LEARN MORE'}</Text>
                  </TouchableOpacity>
                </NativeAsset>
             </NativeAdView>
          </View>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        ) : visiblePosts.length > 0 && containerHeight > 0 ? (
          <FlatList
