...
import { MessageSquare, RefreshCw, Star, ExternalLink, ShieldAlert, Sparkles, Check, Gift, Wrench } from 'lucide-react';
      ...
      <div className="absolute top-0 left-0 right-0 z-20 bg-gradient-to-b from-black/80 via-black/40 to-transparent pointer-events-none pt-[env(safe-area-inset-top)] px-4">
        <div className="flex items-center justify-between pointer-events-auto mb-2 py-2">
           ...
           <div className="flex items-center gap-1.5 sm:gap-2">
             <button
               onClick={() => window.dispatchEvent(new CustomEvent('navigate-service-board'))}
               className="px-2.5 py-1.5 bg-zinc-900/90 border border-yellow-500/60 hover:bg-zinc-800 text-yellow-400 rounded-full font-black italic uppercase text-[10px] tracking-widest flex items-center gap-1 shadow-[0_0_12px_rgba(234,179,8,0.25)] active:scale-95 transition-all"
               title="Find Mechanics & Shops"
             >
               <Wrench size={12} className="text-yellow-400" /> SERVICE
             </button>
             <button
               onClick={() => window.dispatchEvent(new CustomEvent('navigate-giveaway'))}
               className="px-2.5 py-1.5 bg-gradient-to-r from-amber-500 to-orange-600 text-white rounded-full font-black italic uppercase text-[10px] tracking-widest flex items-center gap-1 shadow-[0_0_15px_rgba(245,158,11,0.5)] active:scale-95 transition-transform"
             >
